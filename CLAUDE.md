...
5. **Reuse before reinventing.** Before writing new logic: (a) search codebase for existing hooks/functions/generated JSON; (b) check `docs/_internal/knowledge/methods-index.md` and `field-contracts-index.md`; (c) check `docs/_internal/knowledge/species-profiles.md` and `docs/_internal/knowledge/vehicle-profiles.md`. Duplicating existing data is a defect. **Verbatim consumption is the contract; structural similarity is reuse-name-only.** When a spec cites `file.ext:LINE` as a reuse anchor, the implementation must IMPORT that file's symbols (or modify the file directly), not copy the structure under the same class names while re-implementing the contents. Failure mode: implementation ships an `organ-tbl` class + colgroup-with-percentage-widths matching a spec that explicitly required the named `col-w-*` classes from `OrganBlock.tsx:120` -- all gates pass on WHAT (class name present) without checking WHERE-FROM (cited file imported). Mechanical guard: `scripts/audit-spec-reuse.py` parses spec citations and emits `REUSE-ANCHOR-DRIFT` when imports are missing; pre-commit Step 0g surfaces drift; baseline at `.lattice/reuse-anchor-baseline.json` tolerates pre-existing tech-debt while new drift fails. Companion: frontend-ui-gate.md Rule 6 lists default components (`Table`, `DoseHeader`, `EChartsWrapper`, etc.) that must be imported rather than rebuilt; raw `<table>` / `<button>` / `<select>` JSX where the default exists is the same defect category.
...
| Mechanical rules dispatcher (rules 4, 8, 1-parent, 15) | `.lattice/rule-attestations.yaml` + `scripts/check-rule-attestations.py` | `.githooks/pre-commit` Step 0h BLOCKS on `block`/`any-touch` violations; `.githooks/commit-msg` BLOCKS on `trailer`/`trailer-forbidden` violations. Bypass via `LATTICE_RULE_ATTESTATIONS_SKIP=<id,id>`. Submodule-side rules (3, 6, 11, 17, 1-submodule) require deploying the dispatcher inside `docs/_internal` (TODO entry). |
| Hook deployment | `.githooks/` (per `git config core.hooksPath`) | All framework gates active: Step -1 lock, -0.5 intent, 0 review-gate, 0c F3, 0d F6, 0e F4, 0h rule-attestations, 1+2 tests/build, 5 retro, 6 staging-drift |
