...
| **AUDIT-22** | Expand `onset_concordance` per-study | PARTIAL 2026-04-30 -- intra-study sister-marker expansion in PC + first cross-study extension shipped (5 new assertions, 2 MATCH + 3 SCIENCE-FLAG). PC HIGH ALT (cohort M 1.34x / F 1.25x, engine 1/29) reproduces Stream 6 at sister hepatic marker. PC HIGH ALP (cohort M 1.29x / F 1.53x g=2.81, engine **0/29**) demonstrates worst-case Stream 6: complete miss despite F cohort ratio exceeding AST. TOXSCI-96298 HIGH CHOL M (cohort 1.68x g=2.70 p<0.001, engine **0/30**) confirms Stream 6 reproduces across study + compound (rat Compound A vs PC). Counter-example pin: TOXSCI-96298 HIGH NAG F (cohort 1.84x, engine 6/30) -- engine 2x rule fires correctly when cohort effect is large enough to push individual subjects past threshold; documents the engine's design intent vs. the gap. **Still open:** TOXSCI-87497 (Stream 6 doesn't reproduce -- VOLUME 2.48x M / 2.17x F crosses 2x cleanly; chemistry signals are sub-2x but toxicologist also doesn't call them adverse, so no SCIENCE-FLAG warranted -- documented as findings note here, not authored to yaml). Cross-species (dog) extension via TOXSCI-43066 / TOXSCI-35449 deferred to next cycle per 2-items-per-cycle cadence. |
