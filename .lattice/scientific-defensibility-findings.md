...
| **AUDIT-22** | Expand `onset_concordance` per-study | SUBSTANTIAL 2026-04-30 -- intra-study sister-marker (PC) + cross-study (TOXSCI-96298) + cross-species (TOXSCI-35449 + TOXSCI-43066 dog) shipped across two cycles (9 new assertions total: 3 MATCH + 6 SCIENCE-FLAG). **Cycle 1 (rat sister + cross-study):** PC HIGH ALT (cohort M 1.34x / F 1.25x, engine 1/29 SCIENCE-FLAG sister) + PC HIGH ALP (cohort M 1.29x / F 1.53x, engine **0/29** worst-case complete miss) + TOXSCI-96298 HIGH CHOL M (cohort 1.68x g=2.70 p<0.001, engine **0/30** cross-study) + 2 MATCHes (PC ALT regression PC201708-4009 + TOXSCI-96298 NAG F-dominant 6/30 counter-example). **Cycle 2 (cross-species dog):** TOXSCI-35449 HIGH AST (cohort F 1.95x g=1.29 p=0.051 / M 1.30x, engine 2/10 SCIENCE-FLAG cross-species reproduction) + TOXSCI-43066 HIGH ALP M (cohort M 0.62x g=-2.29 38% decrease, engine **0/10** SCIENCE-FLAG NEW direction-handling dimension) + TOXSCI-43066 HIGH ALT F (cohort F 0.71x g=-1.99 29% decrease, engine **0/10** SCIENCE-FLAG sister-marker direction-handling) + 1 MATCH (TOXSCI-35449 ALP 5/10 by day 28 cross-species counter-example at cohort M 2.12x). NEW Stream 6 dimension: 43066 surfaces engine's `abs(val) > 2.0 * abs(ctrl_mean)` direction-handling blind spot at `onset_recovery.py:140` -- cohort DECREASES are structurally invisible regardless of magnitude (AUDIT-21 named sub-mechanism). Stream 6 blast radius confirmed across: rat increases (PC + 96298) + dog increases (35449) + dog decreases (43066). **Documented but not authored:** TOXSCI-87497 (Stream 6 doesn't reproduce -- VOLUME crosses 2x cleanly, chemistry sub-2x not toxicologist-flagged). **Still open:** corpus-wide expansion to other multi-timepoint LB-bearing studies (instem, Nimble, PDS, etc.) -- substantial closure marker stays open until corpus coverage is complete. |
