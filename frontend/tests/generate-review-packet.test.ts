 ...
 * Output:   docs/scientific-logic.md
 */
...
function generateReviewDocument(): string {
  ...
  lines.push(`syndromes: ${syndromes.length}`);
  ...
  lines.push("");
  lines.push("All p-values and effect sizes in this document are computed by the backend analysis engine. Method documentation: `docs/methods.md`.");
  lines.push("");
...
describe("Scientific Logic Review Packet", () => {
  test("generates review document", () => {
    ...
    // Write to file
    const outPath = resolve(__dirname, "../../docs/scientific-logic.md");
