...
interface FindingsFilterBarProps {
  ...
  onClearScope?: () => void;
  /** Whether the Clinical S2+ rail filter is active. */
  clinicalFilterActive?: boolean;
  /** Called when the user clears the clinical filter chip. */
  onClearClinicalFilter?: () => void;
}
      ...
      {clinicalFilterActive && onClearClinicalFilter && (
        <span className="flex items-center gap-1 rounded-full border bg-accent/50 px-2 py-0.5 text-[10px] font-medium">
          ...
          <button
            ...
            onClick={onClearClinicalFilter}
            title="Clear clinical filter"
          >
