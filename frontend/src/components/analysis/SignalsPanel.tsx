...
function SignalsOrganRailItem({ organ, organBlock, isSelected, maxEvidenceScore, tierDotColor, onClick }: {
  organ: TargetOrganRow; organBlock: OrganBlock | undefined; isSelected: boolean; maxEvidenceScore: number; tierDotColor: string | null; onClick: () => void;
}) {
  ...
  return (
    <button className={cn("group/rail w-full text-left border-b border-border/40 px-3 py-2.5 transition-colors", organ.target_organ_flag ? "border-l-2 border-l-[#DC2626]" : "border-l-2 border-l-transparent", isSelected ? "bg-blue-50/60 dark:bg-blue-950/20" : "hover:bg-accent/30")} onClick={onClick}>
      ...
      <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] text-muted-foreground">
        ...
        {organ.domains.map((d) => (<span key={d} className="inline-flex items-center gap-1 rounded border border-border px-1 py-0.5 text-[9px] font-medium text-foreground/70"><span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: getDomainDotColor(d) }} />{d}</span>))}
      </div>
...
export function SignalsOrganRail({ organs, organBlocksMap, selectedOrgan, maxEvidenceScore, onOrganClick, ruleResults, width }: {
  organs: TargetOrganRow[]; organBlocksMap: Map<string, OrganBlock>; selectedOrgan: string | null; maxEvidenceScore: number; onOrganClick: (organ: string) => void; ruleResults?: RuleResult[]; width?: number;
}) {
  ...
  return (
    <div className="flex shrink-0 flex-col overflow-hidden border-r" style={{ width: width ?? 300 }}>
      ...
      <div className="flex-1 overflow-y-auto">
        {filtered.map((organ, i) => (<div key={organ.organ_system}>{i === separatorIdx + 1 && separatorIdx >= 0 && (<div className="border-b px-3 py-1.5 text-[9px] uppercase tracking-wider text-muted-foreground/50">Other organs</div>)}<SignalsOrganRailItem organ={organ} organBlock={organBlocksMap.get(organ.organ_system)} isSelected={selectedOrgan === organ.organ_system} maxEvidenceScore={maxEvidenceScore} tierDotColor={tierDotMap.get(organ.organ_system) ?? null} onClick={() => onOrganClick(organ.organ_system)} /></div>))}
...
function SignalsOverviewTab({ organ, signalData, ruleResults, modifiers, caveats, onOrganSelect, studyId }: {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="shrink-0 border-t px-4 py-2 flex gap-3">
        <button className="text-xs text-blue-600 hover:underline" onClick={() => navigate(`/studies/${studyId}/target-organs`, { state: { organ_system: organ.organ_system } })}>Target Organs &rarr;</button>
        <button className="text-xs text-blue-600 hover:underline" onClick={() => navigate(`/studies/${studyId}/dose-response`, { state: { organ_system: organ.organ_system } })}>Dose-response &rarr;</button>
        <button className="text-xs text-blue-600 hover:underline" onClick={() => navigate(`/studies/${studyId}/histopathology`, { state: { organ_system: organ.organ_system } })}>Histopathology &rarr;</button>
      </div>
    </div>
  );
}
