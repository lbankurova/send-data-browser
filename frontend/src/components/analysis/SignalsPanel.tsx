  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="shrink-0 border-t px-4 py-2 flex flex-wrap gap-3">
        <button className="text-xs text-blue-600 hover:underline" onClick={() => navigate(`/studies/${studyId}/target-organs`, { state: { organ_system: organ.organ_system } })}>Target Organs: {titleCase(organ.organ_system)} &rarr;</button>
        <button className="text-xs text-blue-600 hover:underline" onClick={() => navigate(`/studies/${studyId}/dose-response`, { state: { organ_system: organ.organ_system } })}>Dose-response: {titleCase(organ.organ_system)} &rarr;</button>
        <button className="text-xs text-blue-600 hover:underline" onClick={() => navigate(`/studies/${studyId}/histopathology`, { state: { organ_system: organ.organ_system } })}>Histopathology: {titleCase(organ.organ_system)} &rarr;</button>
        <button className="text-xs text-blue-600 hover:underline" onClick={() => navigate(`/studies/${studyId}/noael-decision`, { state: { organ_system: organ.organ_system } })}>NOAEL Decision &rarr;</button>
      </div>
    </div>
  );
}
            ...
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id} className="border-b bg-muted/50">
                {hg.headers.map((header) => (
                  <th key={header.id} className="relative cursor-pointer px-2 py-1.5 text-left font-medium hover:bg-accent/50" style={{ width: header.getSize() }} onClick={header.column.getToggleSortingHandler()}>
