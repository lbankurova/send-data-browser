  ...
  return (
    <button className={cn("group/rail w-full text-left border-b border-border/40 px-3 py-2.5 transition-colors", organ.target_organ_flag ? "border-l-2 border-l-[#DC2626]" : "border-l-2 border-l-transparent", isSelected ? "bg-blue-50/60 dark:bg-blue-950/20" : "hover:bg-accent/30")} onClick={onClick}>
      ...
      <div className="mt-1.5 flex items-center gap-2">
        <div className="h-1.5 flex-1 rounded-full bg-muted/50"><div className="h-full rounded-full bg-[#D1D5DB] transition-all" style={{ width: `${barWidth}%` }} /></div>
        <span className={cn("shrink-0 text-[10px] group-hover/rail:text-[#DC2626]", organ.evidence_score >= 0.5 ? "font-semibold" : organ.evidence_score >= 0.3 ? "font-medium" : "")}>{organ.evidence_score.toFixed(2)}</span>
      </div>
