...
| # | Issue | File | Severity | Source |
|---|-------|------|----------|--------|
| ~~HE-1~~ | ~~IMP-02: HCD classification delegated to caller, not computed in query function~~ | — | ~~Low~~ | **RESOLVED (Tier 3A)** — A-3 HCD now computed by `hcd.py:assess_a3()`, integrated into classification pipeline via `assess_finding_with_context()`. OM domain only (Phase 1). |
