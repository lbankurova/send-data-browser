...
## Post-implementation review (2026-03-10)

**All 16 acceptance criteria pass.** Two spec items were intentionally not implemented:

1. **Loading skeleton (Phase 4.2)** — Spec called for "skeleton shimmer while hook fetches." Implementation hides the pane until data arrives. Acceptable: the fetch is small/fast (reshaping precomputed data), and showing a skeleton for a sub-second load adds visual noise without helping the user.

2. **"No individual data" fallback message** — Spec called for "Correlations require individual animal data (not available)." Implementation hides the pane entirely when no individual-basis correlations exist. Acceptable: hiding is better UX than showing a "can't do this" message in a pane the user didn't ask for.

System doc: `docs/systems/correlation-matrix.md` (created).
Follow-on TODOs: GAP-62 (volcano, resolved), GAP-63 (NOAEL, resolved), GAP-64 (BW gain/ratios, open).
