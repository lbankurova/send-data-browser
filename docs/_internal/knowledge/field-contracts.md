...
- `S0_Death` requires `mortalityContext.deathsInSyndromeOrgans > 1` OR `deathsInSyndromeOrgans === 1` with corroborating treatment-related deaths (REM-32). Single uncorroborated death → `S3_Adverse`.
...
- `S1_Monitor` requires `adversity.overall === "non_adverse"` OR `certainty === "insufficient_data"` (data gaps preclude concern-level classification).
...
**Type:** `"mechanism_confirmed" | "mechanism_uncertain" | "pattern_only" | "insufficient_data"`
**Scope:** syndrome-level
...
- Order: `insufficient_data < pattern_only < mechanism_uncertain < mechanism_confirmed`.
...
- Four caps can only **reduce** certainty, never raise it:
  ...
  3. **Data sufficiency** (REM-15): missing confirmatory domain (MI for XS01/XS03/XS04/XS07) → `insufficient_data`. Missing supporting domain (LB for XS10) → `mechanism_uncertain`.
...
- **Syndrome interaction** (REM-31): if XS08 co-detected AND XS07 detected → XS07 certainty capped at `mechanism_uncertain` unless non-thymic lymphoid depletion MI is present.
...
- **Reversibility** (REM-33): full reversal (`reversible === true`) downgrades adversity from `adverse` to `equivocal` for any magnitude (not just minimal/mild).
...
- From pairwise tests (Welch's t for continuous, Boschloo's exact for incidence; Fisher's available as override).
- Not Bonferroni-corrected for continuous domains. Incidence domains are also uncorrected.
...
**Invariants:**
- Never null. Always ≥ 0.
- Composite score combining: severity weight (adverse=3, warning=1), p-value weight (-log10), effect size magnitude (capped at ±5), treatment-related boost (+2), pattern weight (per PATTERN_WEIGHTS), syndrome boost (+3/6 if in syndrome), coherence boost (+2 per additional domain, capped at 6), clinical additive (S4=+5, S3=+3, S2=+2, S1=0; GAP-123), sex concordance/divergence boost (organ-specific 0.3–2.0; GAP-123), clinical floor (S4=15, S3=8, S2=4, S1=0), and confidence multiplier (HIGH=1.0, MODERATE=0.7, LOW=0.4).
- Result is `max(base + synBoost + cohBoost + clinicalAdditive + sexConcordanceBoost, clinicalFloor)`.
...
| `direction_of_concern` | backend | Expected toxicological direction of concern for the endpoint's test code ("up" or "down"), from `BIOMARKER_MAP` in `send_knowledge.py`. Null for non-LB/non-mapped endpoints. |
| `direction_aligns_with_concern` | backend | Boolean: true when observed `direction` matches `direction_of_concern`, false when opposite, null when no concern direction or no observed direction. Feeds D7 confidence dimension. |
