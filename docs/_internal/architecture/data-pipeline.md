...
| `finding_dose_trends.json` | specimen x finding (MI/MA) | ca_trend_p, severity_trend_rho/p, dose_response_pattern, onset_dose_level, pattern_by_sex | ~120 |
...
**HCD reference ranges (Phase 2 — SQLite-first, JSON fallback):** Two-tier HCD architecture. Primary: `backend/data/hcd.db` (SQLite, built by `etl/hcd_etl.py` from NTP DTT IAD organ weight data). 78K+ individual control animal records, 188 pre-aggregated entries across 7 strains × 16 organs × 3 duration categories (28-day, 90-day, chronic). Query layer: `services/analysis/hcd_database.py::HcdSqliteDB` with progressive filter relaxation (strain+sex+organ+duration+route+vehicle → drop vehicle → drop route → base aggregates). Percentile ranking against individual animal distributions. Fallback: `shared/hcd-reference-ranges.json` — static ranges for SD rat (Envigo C11963, 10 organs × 2 sexes × 2 durations) and Wistar Han (NTP TR-587/591/593 composite, 7 organs × 2 sexes, 90-day only). Both loaded by `services/analysis/hcd.py` (`_load_sqlite_db()` tries SQLite first; `_load_db()` loads JSON fallback). Strain alias resolution via `strain_aliases` table (9 canonical strains, ~30 aliases). Used for A-3 factor: treated-group mean compared against [mean±2SD] range. Extended fields when SQLite-sourced: `n`, `study_count`, `source`, `percentile_rank`. OM two-gate modifier: within_hcd + both gates → downgrade to equivocal; outside_hcd + stat only + small magnitude → upgrade to equivocal.
...
**File:** `generator/view_dataframes.py::_compute_signal_score(p_value, trend_p, effect_size, dose_response_pattern, data_type, params)`

**Range:** 0.0 to 1.0. Computed for each endpoint x dose combination.

**Configurable:** All weights, pattern scores, thresholds, and NOAEL penalties are configurable per-study via `ScoringParams` (loaded from `annotations/{study_id}/threshold_config.json`). The defaults below match the original hardcoded values.

**Continuous data (default weights):**
...
**Incidence data (default weights):**

| Component | Weight | Scaling |
|-----------|--------|---------|
| p_value | 0.45 | `-log10(p) / 4.0`, capped at 1.0 |
| trend_p | 0.30 | `-log10(p) / 4.0`, capped at 1.0 |
| dose_response_pattern | 0.25 | Lookup table |
| MI severity modifier | 0.10 | `(sev_grade - 1) / 4.0`, capped at 1.0 |
...
**ANOVA/Dunnett's computation (BUG-04/SD-09 resolved):** The generator's enrichment phase (`domain_stats.py`) computes ANOVA, Dunnett's, and Jonckheere-Terpstra tests from raw per-subject values (`raw_values` key in finding dicts). All continuous domain findings modules (LB, BW, OM, FW) now pass `raw_values` — a list of numpy arrays, one per dose group. The enrichment loop reuses Dunnett and JT results from domain modules when available: for non-OM domains, `dunnett_p` is extracted from the existing `pairwise` field (same computation), and `jt_p` reuses `trend_p`. OM is excluded from Dunnett reuse because its `pairwise` is computed on the recommended normalization metric (ratio_to_bw/ratio_to_brain/ancova) which may differ from `raw_values` (always absolute). ANOVA is always computed fresh. A fallback approximation from `min_p_adj`/`trend_p` exists only for edge cases where raw values are unavailable. The `raw_values` key is popped from finding dicts before JSON serialization.

**Parallel domain execution:** `compute_all_findings()` submits all domain modules across all 3 passes (all-animals, scheduled-only, main-only) to a `ProcessPoolExecutor(max_workers=4)`. This bypasses the GIL for scipy-heavy computations (Dunnett's, Williams', ANCOVA). Wall time is limited by the slowest single domain (typically OM at ~105s for large studies with many dose groups).
...
- Signal score weights, pattern scores, target organ thresholds, and NOAEL penalties are **configurable per-study** via `ScoringParams` (loaded from annotations/threshold-config). Defaults: continuous 0.35/0.20/0.25/0.20, incidence 0.45/0.30/0.25 + 0.10 MI severity modifier, target organ evidence >= 0.3 AND n_significant >= 1.
...
- 2026-03-03: Phase 3 settings enabled (commits `1b7cc1b`, `3f44bba`). Four new post-processing transforms in `parameterized_pipeline.py`: `apply_pairwise_williams()` (Williams' step-down test via `williams.py`), `apply_trend_williams()` (Williams first-step p → trend_p), `apply_organ_weight_method()` (OM alternative stats swap with round-trip), `classify_severity(threshold=...)` (3 adversity threshold modes). Multiplicity correction skipped when Williams is selected (FWER inherent). Transform order: organ_weight → pairwise_williams → trend_williams → rederive_enrichment(threshold). 8/10 settings now active; 3 remain placeholder (Steel, Cuzick, logistic-slope). 16 tests in `test_phase3_transforms.py`.
