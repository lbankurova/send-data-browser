...
- **Line 2:** `mt-1 space-y-0.5 text-[11px] text-muted-foreground` — "Groups: {doseLabels}" comma-joined ascending.
...
- **Line 1 — Stage/NOAEL/LOAEL:** `flex items-baseline gap-1.5 text-xs` — pipeline stage (`font-semibold`, title-cased with underscores replaced), NOAEL (`font-semibold`), LOAEL (`font-semibold`), separated by `|` dividers. Sex qualifier in `text-[11px] text-muted-foreground`.
- **Metrics block:** `mt-1 space-y-0.5 text-[11px] text-muted-foreground`:
...
Sub-header: `text-[11px] font-medium text-muted-foreground` — "Tissue battery:" with inline reference counts (Terminal + Recovery tissue counts per sex) and `"— all animals meet expected count"` when no issues.
...
**Table:** `max-h-72 overflow-auto rounded-md border`, `w-full text-[11px]`:
...
Single `CollapsiblePane` containing all configurable analysis and statistical parameters. Settings use `SettingsRow` + `SettingsSelect` components. Explanatory text below each dropdown in `pl-[7.75rem] text-[11px] leading-snug text-muted-foreground`.
