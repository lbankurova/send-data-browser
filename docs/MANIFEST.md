...
| `systems/recovery-comparison.md` | `backend/routers/temporal.py`, `frontend/src/components/analysis/panes/RecoveryPane.tsx`, `frontend/src/lib/temporal-api.ts`, `frontend/src/hooks/useRecoveryComparison.ts` | 2026-03-10 | Current — continuous domains (all _DOMAIN_COLS: BW/LB/OM/VS/FW/EG/BG) + incidence domains (CL). Per-subject MI examination counting (replaces MA heuristic). Fix 5: low-N confidence on continuous verdicts. |
...
| `knowledge/methods.md` | `frontend/src/lib/*.ts`, `backend/generator/*.py`, `backend/services/analysis/*.py` | 2026-03-03 | Current — analytical methods registry (STAT-*, CLASS-*, METRIC-*, METH-*, DATA-*). CLASS-19 updated for v3 recovery duration table (14 organs, 56 findings, 4 severity models, uncertainty bands). |
...
| `incoming/correlation-context-strategy.md` | Analysis | Correlation pane strategy across context panel levels (endpoint, organ, syndrome) |
| `incoming/organ-correlation-matrix.md` | Implemented | Organ-system correlation matrix in OrganContextPanel — spec for implemented feature |
