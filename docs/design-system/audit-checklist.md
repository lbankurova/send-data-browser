...
|----|------|------|----------|
| C-01 | **Color is signal, not decoration.** Every colored element must encode signal strength (severity, p-value, effect size, score) or be a documented conclusion (NOAEL, target organ, tier). | For each colored element: ask "is this a conclusion or signal?" If categorical identity (workflow state, domain in table, dose group in table, sex in table) → FAIL. | Critical |
...
| C-04 | **Quiet context panel.** Context panel panes use font-weight (`font-semibold`, `font-medium`) and `font-mono` for emphasis, never color. | Scan context panel code for colored text (`text-red-*`, `text-green-*`, etc.) that isn't a severity badge or tier dot. Present → FAIL. | Critical |
| C-05 | **No colored badges for categorical identity in tables.** Workflow state, domain (as badge), dose group, sex — these never get per-category colored badges in tables. | Find badge components in table cells. If they map category → different hue → FAIL. Exception: severity badges (Error/Warning/Info) = signal. | Critical |
...
| C-27 | **Workflow status badges: neutral gray.** Fix status and review status badges all use `bg-gray-100 text-gray-600 border-gray-200`. | Check badge color maps. Per-status colors → FAIL. | High |
