...
| H-003 | Domain labels | Colored text only — `getDomainBadgeColor(domain).text` with `text-[10px] font-semibold`. No dot badges, outline pills, or bordered badges. | Domain codes are categorical identity; a single colored-text treatment keeps them lightweight and consistent. | — |
...
| D-009 | Form controls | Default textarea style: `rounded-md border border-border/50 bg-background px-3 py-1.5 text-xs text-foreground placeholder:text-muted-foreground/50 focus:border-primary focus:outline-none resize`. Border at 50% opacity keeps the control visible but unobtrusive. | Muted border reduces visual weight so the control doesn't compete with data content. Resize enabled by default for user flexibility. | 2026-02-10 |
