...
### METH-08a — TK Satellite Detection and Statistical Exclusion

**Purpose:** Identify TK (toxicokinetic) satellite animals and exclude them from all group-level statistical analyses. TK animals exist solely for plasma exposure data — including them inflates group N, corrupts p-values, incidence denominators, and group means.

**Implementation:** `_parse_tx(study)` returns `(tx_map, tk_setcds)` — backend `dose_groups.py:9`. `build_dose_groups()` maps `is_satellite` via `DM.SETCD.isin(tk_setcds)` — backend `dose_groups.py:131`. All 12 domain findings modules, mortality, and food consumption filter `~subjects["is_satellite"]`.

**Parameters:** Waterfall detection (first match wins):
1. Any `TXPARMCD` starting with "TK" where value is not in `{"non-tk", "none", "no", "n/a", ""}` (case-insensitive)
2. `"TK"` substring in SETCD string (case-insensitive)
3. SETLBL or GRPLBL contains "satellite" or "toxicokinetic" (case-insensitive)

TK sets are excluded from `tx_map` to prevent ARMCD collision (TK arms share ARMCD with main arms). Subject classification uses `DM.SETCD` membership, not ARMCD lookup. Fallback: ARMCD-based detection if DM lacks SETCD column.

**Why this method:** TK animals are scientifically irrelevant to efficacy/safety endpoints — they have no terminal sacrifice data (histopath, organ weights) and their inclusion would dilute treatment effects by inflating denominators. ARMCD-based detection fails because TK and main arms share ARMCD values (e.g., ARMCD "2" maps to both "Group 2, main" and "Group 2, TK"). SETCD-based detection using DM domain membership is the only reliable method.

**Alternatives considered:** ARMCD-based detection (fails due to ARMCD collision — last-writer-wins in tx_map). Checking only `TXPARMCD == "TKGRP"` (fails for studies like PointCross that use `TKDESC` instead). Checking presence of any TK-prefixed param (false positives — `TKDESC="non-TK"` appears in all sets).
...
### METH-32 — Food Consumption Per-Sex Key Stats Derivation

**Purpose:** Extract per-sex BW/FC/FE metrics at highest dose from FC period data, with per-sex recovery derivation, for the Food Consumption context pane.

**Implementation:** `FoodConsumptionPane` `keyStats` useMemo — frontend `SyndromeContextPanel.tsx`.

**Algorithm:**
1. Take the terminal period (`periods[last]`).
2. Group `by_dose_sex` entries by sex. For each sex independently:
   - Find the highest dose level within that sex's entries (dose groups can differ by sex).
   - Extract `bw_pct_change`, `fw_pct_change` from the highest-dose entry.
   - Compute FE pct change: `((mean_food_efficiency - food_efficiency_control) / food_efficiency_control) * 100`, rounded.
3. Recovery per-sex: if a recovery period exists in period data, use it (BW recovered = |bw_pct_change| < 5%, FC recovered = |fw_pct_change| < 5%). Otherwise fall back to study-level `rawData.recovery.bw_recovered` / `fw_recovered`.

**Thresholds for emphasis (font-weight signal):**
- BW/FC: |pct| ≥ 10% → emphasized (font-medium text-foreground)
- FE: |pct| ≥ 20% → emphasized
- Recovery "no" → emphasized; "yes" → muted
...
**Purpose:** Frontend pattern classification for incidence-type findings with histopathology-specific logic and confidence modifiers. Now serves as fallback — backend pattern (CLASS-02 applied per-sex) is preferred when available.

**Implementation:** `classifyPattern(groups, trendP, config)` — frontend `pattern-classification.ts:158`. Called by `classifyFindingPattern()` which first checks for a backend-supplied pattern via `mapBackendPattern()`.

**Backend-first flow (2026-03-25):** `classifyFindingPattern()` accepts optional `backendPattern`/`backendOnset`. When backend provides a pattern via `finding_dose_trends.json` (BFIELD-88/89), the frontend maps it to a `PatternType` and uses it directly — skipping CLASS-03. Fallback to CLASS-03 when: backend pattern is absent, unmapped, or CONTROL_ONLY (frontend-only concept). See `docs/systems/pattern-classification-and-override.md`.
...
**Why backend-first:** Backend per-sex classification uses wider binomial tolerance (n=10) than frontend combined-sex (n=20), correctly absorbing noise. Three divergence sources eliminated: data granularity, tolerance width, algorithm structure.
...
### CLASS-10b — Recovery Verdict (Continuous Endpoints)

**Purpose:** Classify whether a continuous endpoint effect (BW, LB, OM, etc.) reversed, persisted, or progressed during the recovery period, using signed Hedges' g trajectories.

**Implementation:** `classifyContinuousRecovery(terminalG, recoveryG, treatedN?, controlN?)` — frontend `recovery-verdict.ts:32`.

**Parameters:**
- `terminalG`: Signed Hedges' g at terminal sacrifice (main-arm treated vs main-arm control)
- `recoveryG`: Signed Hedges' g at recovery sacrifice (recovery-arm treated vs recovery-arm control)
- Near-zero terminal guard: `|terminalG| < 0.01` → classify on recovery alone (delayed onset detection)
- Overcorrection: sign flip AND `|recoveryG| >= 0.5` → "overcorrected"
- `pct = (|terminalG| - |recoveryG|) / |terminalG| * 100` (magnitude reduction percentage)
- Sub-threshold branch: `|recoveryG| < 0.5` → "resolved" (pct >= 80 or pct < 0) or "reversed" (otherwise)
- Main branch: worsening (pct < 0), reversed (>= 80%), reversing (>= 50%), partial (>= 20%), persistent (< 20%)
- Confidence: `low` when either arm n < 5

**Known limitations (BUG-21):** (1) Sub-threshold branch ignores sign flips — `+0.78 → -0.19` classified as "reversed" when it should arguably be "resolved." (2) `pct` uses `Math.abs()`, making sign changes invisible to the percentage logic. (3) Cross-arm control baseline shift can create artificial terminal effects. (4) No dose-response awareness — each dose classified independently. Full audit: `docs/knowledge/continuous-recovery-verdict-audit.md`.

**Why this method:** Hedges' g is the standard small-sample effect size for preclinical data (n=5–15). The percentage-based tiers (80/50/20) align with regulatory conventions for partial vs complete recovery. Absolute-value comparison ensures direction-agnostic classification (a decrease in organ weight is as meaningful as an increase).

**Alternatives considered:** Within-subject change (eliminates cross-arm shift but requires longitudinal data). Dose-response-aware classification (architecturally larger, requires all doses as input). Binary recovered/not (loses nuance).
...
### CLASS-21 — Organ Proportionality Index (OPI) Classification

**Purpose:** Quantify whether each organ's weight change is proportionate to body weight loss (secondary/expected) or disproportionate (direct toxicity) when XS09 ("Target organ wasting") is active. OPI = (organ weight Δ% from control) / (body weight Δ% from control), computed per-sex with sex-matched BW denominators.

**Implementation:** `computeOrganProportionality(findings, recoveryRows, driver?)` — frontend `organ-proportionality.ts:148`. Pure computation module, no React dependencies. Consumes `UnifiedFinding[]` from existing `useFindings()` hook (domains BW, OM, MI) plus `RecoveryRow[]` from `useRecoveryComparison()`.

**Parameters:**

| OPI Range | Classification | Interpretation |
|-----------|----------------|----------------|
| 0.7–1.3 | `proportionate` | Weight change tracks BW loss — secondary effect |
| > 1.3 | `disproportionate` | Weight change exceeds BW loss — possible direct toxicity |
| 0.3–0.7 | `partially_proportionate` | Intermediate — warrants attention |
| < 0.3 or negative | `inverse` | Organ changes opposite to BW direction — strong direct effect |
| BW Δ% < 5% | `not_applicable` | Insufficient BW change for meaningful ratio |

**Sub-computations:**
- **Per-dose BW delta:** `computeBwDeltaByDoseSex()` extracts BW Δ% at each dose level (not just terminal), stored in `Map<"doseLevel|sex", delta%>`. `computeByDose()` uses per-dose BW Δ% as denominator, falling back to terminal.
- **Paired organ normalization:** `normalizePairedOrgan()` maps plural/variant names to canonical forms (kidneys→kidney, testes→testis, etc.) via `PAIRED_ORGAN_ALIASES`.
- **MI cross-reference:** For each OM organ, finds matching MI findings by specimen. Detects atrophy/degeneration terms. Classifies concordance into 6 types: `concordant_proportionate`, `concordant_disproportionate`, `discordant_weight_only`, `discordant_mi_only`, `mi_not_collected`, `mi_only`.
- **Sex divergence detection:** `checkSexDivergence()` — flagged organ counts differ by >1 OR BW Δ% diverges by >10 percentage points between sexes.
- **Recovery resolution:** Matches organ from `RecoveryRow[]` where `endpoint_label` contains organ name. Resolution% = `((terminal effect - recovery effect) / terminal effect) × 100`. Thresholds: ≥80% → resolved, 30–80% → partial, <30% → persistent.
- **Narrative generation:** Sex-divergence-aware text. Pooled when sexes agree, per-sex when they diverge. Includes optional driver suffix from FC assessment (e.g., "driver: secondary to FC").
- **6 edge-case caveats:** BW >30% loss, sex-divergent BW, brain OPI <0.3, uterus without estrous staging, adrenal+thymus stress pattern, organ sex-divergent classification.

**Why this method:** OPI is the standard quantitative approach toxicologists use to distinguish secondary organ weight changes (proportionate to body weight loss, expected in wasting) from primary organ toxicity (disproportionate, requiring separate investigation). Per-sex computation is critical because M and F body weight trajectories can differ substantially, leading to different denominators and potentially different conclusions per organ.

**Alternatives considered:** Simple threshold on organ weight p-value (doesn't account for the expected decrease proportionate to BW loss). Manual pathologist review without quantitative screening (defeats the "system computes what it can" principle). Pooled-sex OPI (masks sex-specific effects; rejected as scientifically incorrect).
...
- Target organ flag threshold: `evidence_score >= 0.3 AND n_significant >= 1` (configurable via `ScoringParams`)
...
UI display note: The DR&R pane shows the raw A-1 value ("strong" / "weak" / "absent") with a dotted-underline tooltip explaining the classification criteria. Previously the UI mapped "strong" → "monotonic" and "weak" → "non-monotonic", which was misleading — threshold patterns are classified as "strong" but are not monotonic. Fixed in `c09d95f`.
