...
Immunogenicity endpoints use geometric mean titers (GMT) on log10 scale,
...
def compute_is_findings(
    ...
    is_pl = read_xpt_as_polars(study.xpt_files["is"])
    subs = subjects_to_polars(subjects)

    # Use raw titer rows only (not log10 transformed)
    if "ISSCAT" in is_pl.columns:
        titer_vals = is_pl["ISSCAT"].cast(pl.Utf8).str.strip_chars().str.to_uppercase()
        titer_mask = titer_vals.is_in(["TITER", ""])
        if titer_mask.any() and not titer_mask.all():
            is_pl = is_pl.filter(titer_mask)

    # Merge with subjects (main + recovery, exclude TK)
    treatment_subs = subs.filter(~pl.col("is_satellite")).select(["USUBJID", "SEX", "dose_level", "is_recovery"])
    is_pl = is_pl.join(treatment_subs, on="USUBJID", how="inner")

    if is_pl.height == 0:
        return []

    # Convert to pandas for the complex helper functions (BLQ, GMT, seroconversion)
    # The IS domain has extensive iterrows + per-subject calculations that are
    # impractical to port to Polars for marginal benefit on small datasets.
    is_df = is_pl.to_pandas()
    ...
    all_dose_levels = sorted(is_df[~is_df["is_recovery"]]["dose_level"].unique())
    ...
    for tc in test_codes:
        ...
        for sex in sorted(tc_df["SEX"].unique()):
            ...
            time_course = _build_time_course(sex_df, all_dose_levels, all_days, lloq, blq_sub)
            seroconversion = _build_seroconversion(sex_df, all_dose_levels, all_days, lloq, baseline_map)
            peak_day = _find_peak_day(time_course, all_dose_levels)
            group_stats, pairwise_results = _compute_peak_stats(sex_df, all_dose_levels, peak_day, blq_sub)
...
# ---------------------------------------------------------------------------
# Helper functions (unchanged — operate on pandas DataFrames)
# ---------------------------------------------------------------------------
...
def _build_time_course(
    sex_df: pd.DataFrame, dose_levels: list[int], all_days: list[float],
    lloq: float | None, blq_sub: float,
) -> list[dict]:
    result = []
    for day in all_days:
        ...
        for dl in dose_levels:
            ...
            if len(titers) > 0:
                ...
                gmt = ci_lower = ci_upper = None
            groups.append({
                "dose_level": dl, "gmt": round(gmt, 2) if gmt is not None else None,
        ...
        for dl in dose_levels:
            ...
            groups.append({
                ...
                "n": n, "n_blq": n_blq, "is_recovery": True,
            })

        result.append({"day": int(day) if pd.notna(day) else None, "epoch": epoch, "groups": groups})
    return result


def _build_seroconversion(
    sex_df: pd.DataFrame, dose_levels: list[int], all_days: list[float],
    lloq: float | None, baseline_map: dict[str, float],
) -> list[dict]:
    result = []
    for day in all_days:
        day_df = sex_df[sex_df["day"] == day]
        ...
        result.append({"day": int(day) if pd.notna(day) else None, "groups": groups})
    return result
...
def _compute_peak_stats(
    sex_df: pd.DataFrame, dose_levels: list[int], peak_day: float | None, blq_sub: float,
    ...
    for dl in dose_levels:
        ...
        if len(titers) > 0:
            ...
            group_stats.append({
                "dose_level": dl, "n": n,
                "mean": round(gmt, 2), "sd": round(geo_sd, 4) if geo_sd is not None else None,
...
def _compute_trend(peak_df: pd.DataFrame, dose_levels: list[int], blq_sub: float) -> dict:
    main_df = peak_df[~peak_df["is_recovery"]]
    groups = []
    for dl in dose_levels:
        grp = main_df[main_df["dose_level"] == dl]
