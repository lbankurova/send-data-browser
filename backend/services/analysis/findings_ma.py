...
def compute_ma_findings(study: StudyInfo, subjects: pd.DataFrame) -> list[dict]:
    ...
    ma_df["finding_upper"] = ma_df[finding_col].astype(str).str.strip().str.upper()
    ma_abnormal = ma_df[~ma_df["finding_upper"].isin(NORMAL_TERMS)].copy()
    ma_abnormal = ma_abnormal[ma_abnormal["finding_upper"] != "NAN"]

    if len(ma_abnormal) == 0:
        return []
    ...
    findings = []
    grouped = ma_abnormal.groupby([spec_col, finding_col, "SEX"])
